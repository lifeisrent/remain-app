...
### 9-10. 구현 전 확인 질문 (결정 완료, 2026-04-12)
1. Chat 히스토리 저장 범위
   - 결정: **A) 세션 동안만**
2. 1회 응답 길이
   - 결정: **A) 짧게(2~4문장)**
3. quick reply 유지 여부
   - 결정: **제거(단, 코드/구조는 완전 삭제하지 않고 비노출 처리)**
4. 모델 우선순위
   - 결정: **Anthropic 기본**

### 9-11. 구현 지시 (결정 반영)
- ChatTab을 자유 입력 대화형으로 전환
- 응답 길이 제한을 시스템 프롬프트에 명시(2~4문장)
- quick reply UI는 feature flag처럼 숨김 처리
- 세션 리셋 시 대화 히스토리 초기화 (영구 저장 없음)
